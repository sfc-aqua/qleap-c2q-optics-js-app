...
function toCartesian([r, angle]) { // eslint-disable-line no-unused-vars
...
function complexAdd([r1, i1], [r2, i2]) { // eslint-disable-line no-unused-vars
  return [r1 + r2, i1 + i2];
}
class WaveForm {
  // public
  ...
  computeNthTerm(term) {
    ...
    for (let i = 0; i < width; i++) {
      ...
      const [r1, i1] = complexMultiply([points[i], 0], coef);
      sum[0] += r1;
      sum[1] += i1;
    }
    const avg = [sum[0] / width, sum[1] / width];
    return toPolar(avg); // polar coordinates for easy processing with sin cos
  }
